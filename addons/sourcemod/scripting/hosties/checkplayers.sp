...
{
	gH_Cvar_CheckPlayersOn = AutoExecConfig_CreateConVar("sm_hosties_checkplayers_enable", "1", "Enable or disable the !checkplayers command: 0 - disable, 1 - enable", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public Action:Command_CheckPlayers(client, args)
{
	if (gShadow_CheckPlayersOn)
	{
		if (IsPlayerAlive(client))
		{
			...
			{
				CPrintToChat(client, CHAT_BANNER, "No Rebels ATM");
			}
			else
			{
	...
	{
		CReplyToCommand(client, CHAT_BANNER, "CheckPlayers CMD Disabled");
	}
