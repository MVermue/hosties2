...
GunSafety_OnPluginStart()
{
	...
	gH_Cvar_Strip_On_Slay = AutoExecConfig_CreateConVar("sm_hosties_strip_onslay", "1", "Enable or disable the stripping of weapons from anyone who is slain.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	gShadow_Strip_On_Slay = true;
	gH_Cvar_Strip_On_Kick = AutoExecConfig_CreateConVar("sm_hosties_strip_onkick", "1", "Enable or disable the stripping of weapons from anyone who is kicked.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	gShadow_Strip_On_Kick = true;
	gH_Cvar_Strip_On_Ban = AutoExecConfig_CreateConVar("sm_hosties_strip_onban", "1", "Enable or disable the stripping of weapons from anyone who is banned.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
...
public MenuHandler_Slay(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		{
			CPrintToChat(param1, CHAT_BANNER, "Player no longer available");
		}
		else if (!CanUserTarget(param1, target))
		{
			CPrintToChat(param1, CHAT_BANNER, "Unable to target");
		}
		else if (!IsPlayerAlive(target))
		{
			CReplyToCommand(param1, CHAT_BANNER, "Player has since died");
		}
		else
		{
			decl String:name[32];
			...
			CShowActivity2(param1, "[SM] ", "%t", "Slayed target", "_s", name);
		}
...
public MenuHandler_Kick(Handle:menu, MenuAction:action, param1, param2)
{
	...
	else if (action == MenuAction_Select)
	{
		...
		else
		{
			...
			GetClientName(target, name, sizeof(name));
			CShowActivity2(param1, "[SM] ", "%t", "Kicked target", "_s", name);
...
PrepareBan(client, target, time, const String:reason[])
{
	...
	if (originalTarget != target)
	{
		...
		{
			CPrintToChat(client, CHAT_BANNER, "Player no longer available");
		}
