...
Rules_OnPluginStart()
{
	...
	gH_Cvar_Announce_Rules = AutoExecConfig_CreateConVar("sm_hosties_announce_rules", "1", "Enable or disable rule announcements in the beginning of every round ('please follow the rules listed in !rules'): 0 - disable, 1 - enable", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	gShadow_Announce_Rules = true;
	
	gH_Cvar_Rules_Mode = AutoExecConfig_CreateConVar("sm_hosties_rules_mode", "1", "1 - Panel Mode, 2 - Website", FCVAR_PLUGIN, true, 1.0, true, 2.0);
	gShadow_Rules_Mode = 2;
	
	gH_Cvar_Rules_Website = AutoExecConfig_CreateConVar("sm_hosties_rules_website", "http://www.youtube.com/watch?v=oHg5SJYRHA0", "The website for the rules page.", FCVAR_PLUGIN);
